...
def test_add_records_param_price(
    brand, expected_price, expectation, populated_datastore
):
    with expectation:
...
def test_add_records_param_rating(
    brand, expected_rating, expectation, populated_datastore
):
    with expectation:
        ds = populated_datastore
...
@pytest.mark.parametrize(
    ...
    [
        (
            Record(name="iphone 15 pro", brand="apple", price=999, rating=4.9),
            does_not_raise(),
        ),
        (
            Record(name="iphone 14", brand="apple", price=-10, rating=4.0),
            pytest.raises(RecordParseError),
        ),
        (
            Record(name="iphone 14", brand="apple", price=100, rating=6.0),
            pytest.raises(RecordParseError),
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        [
            Record(name="iphone 15 pro", brand="apple", price=999, rating=4.9),
            Record(name="redmi note 12", brand="xiaomi", price=199, rating=4.6),
        ],
        [],
    ],
)
